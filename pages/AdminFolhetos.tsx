...
export const AdminFolhetos: React.FC = () => {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      setUploading(true);
      await api.uploadFolheto(selectedFile);
      show('Folheto enviado com sucesso', 'success');
      setSelectedFile(null);
      const fileInput = document.getElementById('file-input') as HTMLInputElement;
      if (fileInput) fileInput.value = '';
      await loadFolhetos();
    } catch (error) {
      console.error('Error uploading folheto:', error);
      show('Erro ao enviar folheto', 'error');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Tem a certeza que deseja eliminar este folheto?')) {
      return;
    }

    try {
      setDeleting(id);
      await api.deleteFolheto(id);
      show('Folheto eliminado com sucesso', 'success');
      await loadFolhetos();
    } catch (error) {
      console.error('Error deleting folheto:', error);
      show('Erro ao eliminar folheto', 'error');
    } finally {
      setDeleting(null);
    }
  };
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Gestão de Folhetos</h1>
            <p className="text-gray-600">Faça upload e gerencie os folhetos disponíveis para todos os perfis</p>
          </div>

        {/* Upload Form */}
        <div className="bg-white rounded-lg border border-gray-200 p-6 mb-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Enviar Novo Folheto</h2>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">Folhetos Carregados</h2>
          </div>
          ...
          {loading ? (
            ...
            <div className="px-6 py-8 text-center text-gray-500">
              Nenhum folheto carregado ainda
            </div>
          ) : (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {folhetos.map((folheto) => (
                    <tr key={folheto.id} className="border-b border-gray-200 hover:bg-gray-50">
                      <td className="px-6 py-4 text-sm text-gray-900">{folheto.original_filename}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{formatFileSize(folheto.file_size)}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{formatDate(folheto.created_at)}</td>
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() => handleDelete(folheto.id)}
                          disabled={deleting === folheto.id}
