...
export const AuditList: React.FC = () => {
  ...
  // Clique numa auditoria (usa PDF se já submetida)
  const handleAuditClick = async (audit: Audit & { store: Store }) => {
    console.log(
      '🔍 handleAuditClick chamado para auditoria:',
      audit.id,
      'Status:',
      audit.status
    );
  ...
  const filteredAudits = useMemo(() => {
    ...
    // Só histórico: auditorias submetidas ou concluídas
    ...
    if (viewMode === 'calendar') {
      filtered = filtered.filter(a => {
        ...
        return (
          auditDate.getMonth() === selectedMonth &&
    ...
    return filtered.sort(
      (a, b) =>
        new Date(b.dtstart).getTime() - new Date(a.dtstart).getTime()
    );
  }, [audits, viewMode, selectedStore, selectedMonth, selectedYear]);

  // Combine audits + visits for calendar tiles
  const combinedItems = useMemo(() => {
    ...
    return [...filteredAudits, ...mappedVisits].sort(
      (a, b) =>
        new Date(b.dtstart).getTime() - new Date(a.dtstart).getTime()
    );
      ...
      <div className="grid grid-cols-7 gap-0 border border-gray-300">
        {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(
          day => (
            <div
              key={day}
              className="bg-gray-100 p-2 text-center text-sm font-semibold text-gray-700 border-b border-gray-300"
            >
              {day}
            </div>
          )
  ...
  const renderStoreView = () => {
    const storeGroups = stores
      .map(store => {
        const storeAudits = filteredAudits.filter(
          a => a.store_id === store.id
        );
        const storeVisits = visits.filter(
          v => v.store_id === store.id
        );
    ...
    return (
      <div className="space-y-4">
        {storeGroups.map(
          ({ store, audits, visits: storeVisits }) => (
            <div
              key={store.id}
  ...
  const renderDOTView = () => {
    ...
    return (
      <div className="space-y-4">
        {dotGroups.map(
          ({ dot, audits, stores, visits: dotVisits }) => (
            <div
              key={dot.id}
              className="bg-white border border-gray-200 rounded-lg p-4"
            >
            ...
            {totalPages > 1 && (
              <div className="mt-4 flex items-center justify-between bg-white px-4 py-3 rounded-lg">
                <div className="text-sm text-gray-700">
                  ...
                  de {filteredAudits.length} resultados
                </div>
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() =>
