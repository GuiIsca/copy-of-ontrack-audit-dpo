...
const EMAIL_DISCLAIMER = `AVISO IMPORTANTE:
Esta comunicação foi enviada através do sistema de contacto administrativo.
Tratamento de dados pessoais: Os seus dados são tratados de acordo com a legislação de proteção de dados em vigor.`;

const ADMIN_EMAIL = 'admin@mousquetaires.com';
...
export const ContactAdmin: React.FC = () => {
  ...
  const handleSendByEmail = () => {
    ...
    if (!selectedDepartment) {
      show('Por favor selecione um departamento', 'error');
      return;
    }

    if (!message.trim()) {
      show('Por favor escreva uma mensagem', 'error');
      return;
    }

    const subject = `Contacto - ${selectedDept?.name || 'Admin'}`;
    const body = encodeURIComponent(
      `${message}\n\n---\n${EMAIL_DISCLAIMER}`
    );

    window.location.href = `mailto:${ADMIN_EMAIL}?subject=${encodeURIComponent(subject)}&body=${body}`;
  };
            ...
            <div className="flex justify-between gap-3">
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={!selectedDepartment || !message.trim()}
